...
export function CameraAnimator({
  ...
  const activeZoomCameraRef = useRef<
  ...
  useEffect(() => {
    if (camera instanceof THREE.PerspectiveCamera || camera instanceof THREE.OrthographicCamera) {
      activeZoomCameraRef.current = camera;
    }
  }, [camera]);
  ...
  useFrame(() => {
    ...
    const activeZoomCamera = activeZoomCameraRef.current;
    ...
    if (activeZoomCamera && typeof preset.zoom === "number") {
      activeZoomCamera.zoom += (preset.zoom - activeZoomCamera.zoom) * 0.08;
      activeZoomCamera.updateProjectionMatrix();
    }
    ...
    const zoomSettled =
      ...
      Math.abs(activeZoomCamera.zoom - preset.zoom) < 0.5;
...
const DAY_NIGHT_PERIOD = 900; // 15 min cycle (slower, less jarring)
