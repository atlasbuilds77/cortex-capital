-- AUTONOMOUS TRADING COMPANY - Complete Database Schema
-- Built: Feb 7, 2026
-- Based on: Voxyz multi-agent system architecture
-- Follows ops_* table naming, JSONB for config, proper indexes, foreign keys with cascading deletes

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ==================== CORE LOOP TABLES (4 tables) ====================
...
CREATE TABLE ops_agent_events (
    ...
    metadata JSONB NOT NULL DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_agent_events_tags GIN (tags)
);

-- ==================== POLICY & CONFIG TABLES (3 tables) ====================
...
CREATE TABLE ops_agent_reactions (
    ...
    INDEX idx_agent_reactions_status (status)
);

-- ==================== MEMORY & LEARNING TABLES (3 tables) ====================
...
CREATE TABLE ops_agent_memory (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_agent_memory_agent_type (agent_id, type),
    INDEX idx_agent_memory_confidence (confidence),
    INDEX idx_agent_memory_tags GIN (tags)
);
...
CREATE TABLE ops_trade_outcomes (
    ...
    metadata JSONB NOT NULL DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_trade_outcomes_outcome_type (outcome_type),
    INDEX idx_trade_outcomes_pnl_percent (pnl_percent)
);
...
CREATE TABLE ops_roundtable_queue (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_roundtable_queue_status (status),
    INDEX idx_roundtable_queue_format (format)
);
...
-- ==================== TRADING SPECIFIC TABLES (3 tables) ====================
...
CREATE TABLE ops_signals (
    ...
    processed_at TIMESTAMP WITH TIME ZONE,
    INDEX idx_signals_token_confidence (token, confidence),
...
CREATE TABLE ops_action_runs (
    ...
    metadata JSONB NOT NULL DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_action_runs_action_type (action_type),
    INDEX idx_action_runs_status (status)
);
...
COMMENT ON TABLE ops_trading_proposals IS 'Trading proposals from agents, can be auto-approved or require roundtable';
COMMENT ON TABLE ops_missions IS 'Approved proposals become missions with concrete steps';
COMMENT ON TABLE ops_mission_steps IS 'Individual steps within a mission (analyze, execute, monitor, etc.)';
COMMENT ON TABLE ops_agent_events IS 'Events generated by agents (trade outcomes, insights, alerts)';
COMMENT ON TABLE ops_policy IS 'System policies controlling auto-approval, limits, risk controls';
COMMENT ON TABLE ops_trigger_rules IS 'Trigger rules for reactive and proactive actions';
COMMENT ON TABLE ops_agent_reactions IS 'Queued agent-to-agent reactions based on events';
COMMENT ON TABLE ops_agent_memory IS 'Agent memories (insights, patterns, strategies, lessons)';
COMMENT ON TABLE ops_trade_outcomes IS 'Completed trade results for learning and analysis';
COMMENT ON TABLE ops_portfolio_history IS 'Historical portfolio snapshots for performance tracking';
COMMENT ON TABLE ops_roundtable_queue IS 'Queued conversations between agents';
COMMENT ON TABLE ops_agent_relationships IS 'Agent affinity scores and interaction history';
COMMENT ON TABLE ops_signals IS 'Trading signals from various sources (KOLs, patterns, indicators)';
COMMENT ON TABLE ops_positions IS 'Open trading positions with P&L tracking';
COMMENT ON TABLE ops_action_runs IS 'System action runs for monitoring and debugging';
