...
function analyzeDay(bars: Bar[]): any {
  ...
  // LONG: Strong uptrend + pullback to EMA9 + bounce  
  // TIGHT FILTERS
  const strongUptrend = ema9 > ema21 && ema21 > sma50 && current.close > sma50;
  const pullbackToEma = prev.low <= ema9 * 1.02 && prev.low >= ema9 * 0.97; // Pullback touched EMA
  const bounceConfirm = current.close > prev.high && current.close > ema9; // Closed above prev high AND above EMA
  const trendConfirm = trendStrength > 3 && trendStrength < 15; // Strong but not extended
  const volConfirm = volRatio > 1.2;
  
  if (strongUptrend && pullbackToEma && bounceConfirm && trendConfirm && volConfirm) {
    return {
...
async function backtest() {
  console.log('LONG-ONLY BACKTEST (TIGHTER FILTERS)\n' + '='.repeat(50));
